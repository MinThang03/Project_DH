...
public class Mainsuatt extends AppCompatActivity {
    ...
    private Button btnAnh1, btnAnh2, btnAnh3, btnLuu;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.themxe);

        // Khởi tạo các EditText và Spinner
        btnLuu = findViewById(R.id.btluu);
        ...
        ArrayAdapter<String> adapter = new ArrayAdapter<>(Mainsuatt.this, android.R.layout.simple_spinner_item, carTypes);
        ...
        String maxe = getIntent().getStringExtra("maxe");
        Cursor cursor = databaseHelper.getsuaxe(maxe);
        if (cursor != null && cursor.moveToFirst()) {
            // Lấy thông tin từ Cursor, kiểm tra nếu cột tồn tại
            String tenXe = getStringColumn(cursor, "Tenxe");
            String hangXe = getStringColumn(cursor, "Hangxe");
            String bienSo = getStringColumn(cursor, "Bienso");
            String gia = getStringColumn(cursor, "Gia");
            String hinhanh = getStringColumn(cursor, "Hinhanh");
            String trangThai = getStringColumn(cursor, "TrangThai");
            String soGhe = getStringColumn(cursor, "Soghe");
            String kieuso = getStringColumn(cursor, "Kieuso");
            String dongCo = getStringColumn(cursor, "Dongco");
            String copXe = getStringColumn(cursor, "Copxe");
            String tocDo = getStringColumn(cursor, "Tocdo");
            String tuiKhi = getStringColumn(cursor, "Tuikhi");
            String moTa = getStringColumn(cursor, "Mota");
            anh1 = getStringColumn(cursor, "Anh1");
            anh2 = getStringColumn(cursor, "Anh2");
            anh3 = getStringColumn(cursor, "Anh3");

            if (tenXe == null || hangXe == null || tenXe.isEmpty() || hangXe.isEmpty()) {
                Toast.makeText(Mainsuatt.this, "Không tìm thấy thông tin xe", Toast.LENGTH_SHORT).show();
            }

            // Set text cho các EditText (nếu có dữ liệu để hiển thị)
            editTextTenxe.setText(tenXe);
            editTextHangxe.setText(hangXe);
            editTextBienso.setText(bienSo);
            editTextGia.setText(gia);
            editTextTrangthai.setText(trangThai);
            editTextSoghe.setText(soGhe);
            editTextKieuso.setText(kieuso);
            editTextDongco.setText(dongCo);
            editTextCopxe.setText(copXe);
            editTextTocdo.setText(tocDo);
            editTextTuikhi.setText(tuiKhi);
            editTextMota.setText(moTa);

            String loaixe = getStringColumn(cursor, "Maloaixe");
            String carTypeName = getCarTypeName(loaixe); // Lấy tên loại xe từ mã
            int carTypePosition = carTypes.indexOf(loaixe + " - " + carTypeName); // Tìm vị trí của loại xe trong Spinner
            if (carTypePosition >= 0) {
                spinnerMaloai.setSelection(carTypePosition); // Đặt lại vị trí của Spinner
            }

            // Hiển thị ảnh từ drawable hoặc bộ nhớ trong
            setImage(anh1, imageViewAnh1);
            setImage(anh2, imageViewAnh2);
            setImage(anh3, imageViewAnh3);
        } else {
            // Nếu không có dữ liệu từ database
            Toast.makeText(Mainsuatt.this, "Không tìm thấy xe với mã xe: " + maxe, Toast.LENGTH_SHORT).show();
        }
        cursor.close();
        ...
        btnLuu.setOnClickListener(v -> {
            ...
            boolean isUpdated = dbHelper.updateXe(maxe, tenXex, loaiXex, hangXex, bienSox, anh1Update, giax, trangThaix, soGhex, kieusox, dongCox, copXex, tocDox, tuiKhix, moTax, anh1Update, anh2Update, anh3Update);
    ...
    private void setImage(String imageNameOrPath, ImageView imageView) {
        if (imageNameOrPath != null && !imageNameOrPath.isEmpty()) {
            int imageResId = getResources().getIdentifier(imageNameOrPath, "drawable",getPackageName());
            ...
            if (imageResId != 0) {
                ...
                if (imgFile.exists()) {
                    ...
                    imageView.setImageBitmap(bitmap); // Set ảnh từ bộ nhớ trong
                } else {
                    // Nếu không tìm thấy ảnh, đặt ảnh mặc định
                    imageView.setImageResource(R.drawable.vvf3);
                }
            }
        } else {
            imageView.setImageResource(R.drawable.vvf6); // Ảnh mặc định nếu không có tên ảnh
        }
    }
